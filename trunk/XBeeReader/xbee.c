...
void xbee_in(xbee_t *xbee, const void *buf, uint8_t len)
{
    ...
    while(len) {
            ...
            case 2:
              xbee->in.hdr_data[xbee->in.bytes_rcvd++] = *data++;
                ...
                if (xbee->in.bytes_left > XBEE_MAX_DATA_LEN
                    || ((xbee->in.packet 
                              = xbee_alloc_pkt_mem(xbee, XBEE_RECV, xbee->in.bytes_left + 4)) == NULL)
                )
                {
        ...
        if (xbee_recv_pkt(xbee, xbee->in.packet, xbee->in.bytes_rcvd)) {
            xbee_free_pkt_mem(xbee, xbee->in.packet);
                  ...
                  const uint8_t params[])
{
    ...
    pkt = (xbee_at_cmd_pkt_t *)xbee_alloc_pkt_mem(xbee, XBEE_XMIT, param_len + 8);
    if (pkt == NULL) {
        xbee_tx_err();
        return -ENOMEM;
    }
    ...
    xbee_free_pkt_mem(xbee, (xbee_pkt_t *)pkt);
                            ...
                            const uint8_t addr16[2])
{
    ...
    pkt = (xbee_remote_at_cmd_pkt_t *)xbee_alloc_pkt_mem(xbee, XBEE_XMIT, param_len + 19);
    if (pkt == NULL) {
        xbee_tx_err();
        return -ENOMEM;
    }
...
int xbee_send64(xbee_t *xbee, const void *data, uint8_t len, uint8_t opt, const uint8_t addr[8])
{
    ...
    pkt = (xbee_a64_tx_pkt_t *)xbee_alloc_pkt_mem(xbee, XBEE_XMIT, len + 15);
    if (pkt == NULL) {
        xbee_tx_err(xbee);
        return -ENOMEM;
    }
...
int xbee_send16(xbee_t *xbee, const void *data, uint8_t len, uint8_t opt, const uint8_t addr[2])
{
    ...
    pkt = (xbee_a16_tx_pkt_t *)xbee_alloc_pkt_mem(xbee, XBEE_XMIT, len + 9);
    if (pkt == NULL) {
        xbee_tx_err(xbee);
        return -ENOMEM;
    }
